...
function DatabaseComponent() {
    ...
    return (
        <>
            <div className="flex items-center mt-12 mb-12 max-w-md mx-auto p-2 bg-white border rounded-lg shadow-md">
                <input
                    id="search"
                    name="search"
                    type="text"
                ...
                />
                <button className="px-3 py-1 rounded-md ml-2 bg-[#2E5B00] hover:bg-[#EFBE55] font-semibold text-white" onClick={(e: any) => {
                    if (currentFilter.search == "") {
                        setCurrentFilter({ ...currentFilter, mode: "default", toggle: !currentFilter.toggle, page: 1 });
                        return;
                    } else if (currentFilter.search.length <= 3) {
            ...
            <div className="relative min-h-[96px] w-full">
                {loading ? (<Loader />) : (
                    <>
                        ...
                        <div className="flex overflow-x-auto mb-12 mt-4 justify-center mx-8">
